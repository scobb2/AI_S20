...
def setup():
    ...
    nnPermutations = [LyrParam(chan1, chan2, chan3, \
     drpt1, drpt2, drpt3, CNNbatchNorm)
...
class LyrParam :
    def __init__(self, chan1, chan2, chan3, drpt1, drpt2, drpt3, CNNbatchNorm):
        ...
        for x in range(repsPerTest):
            ...
            self.nn[x].add(layers.Conv2D(self.chan1, (3, 3), activation = 'relu', \
             input_shape = (28, 28, 1)))  # Add Conv layer
            ...
            hst = self.nn[x].fit(x = generator(train_data, train_labels, batch_size = batchSize), validation_data = (test_data, test_labels),
             epochs = maxNumEpochs, steps_per_epoch = stepsPerEpoch, callbacks = [es, mc], verbose = 1).history # validation_data = validation_generator

            scores = self.nn[x].evaluate(test_data, test_labels, verbose = 10)
            print ( scores )

            print ("Max val accuracy: " + str(max(hst['val_accuracy'])) + \
             " with loss: " + str(min(hst['val_loss'])))
            outFile.write("Max val accuracy: " + str(max(hst['val_accuracy'])) + \
             " with loss: " + str(min(hst['val_loss'])) + "\n")
...
# Image Generation
def generator(images, labels, batch_size = batchSize):
    datagen = ImageDataGenerator (
        rotation_range = 5,
        shear_range = .2,
        zoom_range = .1,
    ...
    for x, y in datagen.flow(images, labels, batch_size = batchSize):
        yield x, y

 # 128 x 28 x 28 x 1
    # for data, labels in datagen.flow(inputs, outputs, batch_size = batchSize):
    #     print (datagen.flow(inputs, outputs, batch_size = batchSize))
    #     yield datagen.flow(inputs, outputs, batch_size = batchSize)

# Create, reshape, and range index new images
datagen.fit(train_data)
# train_generator = generator(train_data, train_labels, batch_size = batchSize)
# validation_generator = generator(train_data, train_labels, batch_size = batchSize)
